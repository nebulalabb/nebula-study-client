...
import CuteModal from './ui/CuteModal';
...
export function SolverHistory({ onSelect, refreshKey }: SolverHistoryProps) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/solver/history/${idToDelete}`);
  ...
  return (
    <>
      ...
      <CuteModal 
        ...
        title="Xóa lịch sử"
        description="Bạn có chắc chắn muốn xóa lời giải này khỏi lịch sử không? Hành động này không thể hoàn tác."
        type="confirm"
      />
    </>
  );
